function [out] = HRV_analysis_old(RRI,arg2)
...
resol = 10;
minL = 4;
maxL = round(length(NN)/4);
plotflag = 0;
if length(NN)>100
    X.alpha = dfa_hr_rr(NN,minL,maxL,resol,plotflag);
else
    X.alpha = NaN;
end
% end
...
%% output
out.NumRRI = X.N;
out.meanNN = X.meanNN'/1000;
out.SDNN = X.SDNN'/1000;
out.RMSSD = X.RMSSD'/1000;
out.pNN50 = X.pNN50';
out.SD1 = X.SD1'/1000;
out.SD2 = X.SD2'/1000;
out.SD1_SD2 = (X.SD1/X.SD2)';
out.VLF = X.VLF';
out.LF = X.LF';
out.HF = X.HF';
out.TotalPower = X.TotalPower';
out.LFHFratio = X.LFHFratio';
out.LFnu = X.LFnu';
out.HFnu = X.HFnu';
out.alpha = X.alpha';